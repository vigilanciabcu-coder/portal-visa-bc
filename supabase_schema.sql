...
-- 8. Tabela de Processos Sanitários / 1Doc
CREATE TABLE IF NOT EXISTS public.processos (
    ...
    data_situacao DATE,
    venc_licenca DATE,
    grau_risco TEXT DEFAULT 'MÉDIO RISCO',
...
-- 9. Tabela de Mural de Recados
CREATE TABLE IF NOT EXISTS public.recados_mural (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    titulo TEXT NOT NULL,
    conteudo TEXT NOT NULL,
    prioridade TEXT NOT NULL DEFAULT 'NORMAL',
    autor TEXT NOT NULL DEFAULT 'DIRETORIA VISA',
    cargo TEXT DEFAULT 'DIRETOR',
    data DATE DEFAULT CURRENT_DATE,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- 10. Tabela de Escritórios de Contabilidade
CREATE TABLE IF NOT EXISTS public.contabilidades (
    ...
    telefone TEXT,
    senha TEXT DEFAULT '123456',
    cnpjs_vinculados JSONB DEFAULT '[]',
    data_cadastro TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- 11. Tabela de Documentos de Contabilidade
CREATE TABLE IF NOT EXISTS public.documentos_contabilidade (
    id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::text,
    contabilidade_id TEXT,
...
-- 12. Tabela de Contribuintes (Empresários, Feirantes e Autônomos)
CREATE TABLE IF NOT EXISTS public.contribuintes (
    id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::text,
    tipo_pessoa TEXT DEFAULT 'PJ',
    categoria TEXT DEFAULT 'EMPRESARIO',
...
-- 13. Tabela de Cidadãos (Munícipes cadastrados)
...
-- 14. Inserir Amostras de Laboratório Iniciais (Exemplo Laudos Oficiais)
...
-- 15. Inserir Usuários Iniciais de Exemplo
...
-- 16. Inserir Contabilidade e Contribuinte Exemplo
INSERT INTO public.contabilidades (id, razao_social, nome_fantasia, cnpj, crc, responsavel, email, telefone, senha, cnpjs_vinculados)
VALUES 
    ('contab-1', 'BALNEÁRIO ASSESSORIA CONTÁBIL LTDA', 'Balneário Contabilidade', '83.102.285/0001-07', 'CRC/SC-012345/O', 'Carlos Alberto Contabilista', 'contato@balneariocontabil.com.br', '(47) 3367-1020', '123456', '["83.102.285/0001-07", "12.345.678/0001-90"]')
ON CONFLICT (id) DO NOTHING;

INSERT INTO public.contribuintes (id, tipo_pessoa, categoria, cnpj_cpf, razao_social, nome_fantasia, responsavel, email, telefone, ramo_atividade, bairro, endereco, senha)
VALUES 
    ('contrib-1', 'PJ', 'EMPRESARIO', '12.345.678/0001-90', 'RESTAURANTE E PIZZARIA ATLÂNTICO LTDA', 'Pizzaria Atlântico BC', 'Marcos Roberto da Silva', 'contato@pizzariaatlantico.com.br', '(47) 3361-9090', '56.11-2-01 - Restaurantes e similares', 'Centro', 'Av. Atlântica, 2400', '123456')
ON CONFLICT (id) DO NOTHING;

-- 17. Habilitar Permissões (Row Level Security - RLS)
...
ALTER TABLE public.recados_mural ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.contabilidades ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.documentos_contabilidade ENABLE ROW LEVEL SECURITY;
...
DROP POLICY IF EXISTS "Permitir Acesso Anonimo Processos" ON public.processos;
CREATE POLICY "Permitir Acesso Anonimo Processos" ON public.processos FOR ALL USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Permitir Acesso Anonimo Mural" ON public.recados_mural;
CREATE POLICY "Permitir Acesso Anonimo Mural" ON public.recados_mural FOR ALL USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Permitir Acesso Anonimo Contabilidades" ON public.contabilidades;
CREATE POLICY "Permitir Acesso Anonimo Contabilidades" ON public.contabilidades FOR ALL USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Permitir Acesso Anonimo Documentos Contabilidade" ON public.documentos_contabilidade;
CREATE POLICY "Permitir Acesso Anonimo Documentos Contabilidade" ON public.documentos_contabilidade FOR ALL USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Permitir Acesso Anonimo Contribuintes" ON public.contribuintes;
CREATE POLICY "Permitir Acesso Anonimo Contribuintes" ON public.contribuintes FOR ALL USING (true) WITH CHECK (true);
