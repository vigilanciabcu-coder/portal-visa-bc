...
import { CadastroContribuinteModal } from './components/CadastroContribuinteModal';
...
import { AlvaraView } from './components/AlvaraView';
import { DemandasFiscaisView } from './components/DemandasFiscaisView';
...
const PORTAL_BUTTONS: PortalButton[] = [
  ...
  { id: 'demandas_fiscal', nome: 'Minhas Demandas (Fiscal)', url: '', img: 'demandas-fiscal', acao: 'view', view: 'demandas_fiscal', badgetext: 'FISCAL', perfisPermitidos: ['SERVIDOR'] },
  { id: 'demandas_diretor', nome: 'Painel da Diretoria (Demandas)', url: '', img: 'demandas-diretor', acao: 'view', view: 'demandas_diretor', badgetext: 'DIRETORIA', perfisPermitidos: ['SERVIDOR'] },
  { id: 'alvara_visa', nome: 'ALVARÁ', url: '', img: 'alvara', acao: 'view', view: 'alvara_visa', badgetext: 'OFICIAL', perfisPermitidos: ['SERVIDOR', 'CONTABILIDADE', 'CIDADAO', 'CONTRIBUINTE'] },
  ...
  { id: 'debi', nome: 'Débitos', url: 'https://cidadao.bc.sc.gov.br/cidadao/balneario_camboriu/portal/servicos/debitos?params=NA%3D%3D', img: 'https://wcbzmpnvcjamlgljsksk.supabase.co/storage/v1/object/public/public-assets/consultadedebitos.avif', acao: 'link', perfisPermitidos: ['SERVIDOR', 'CONTABILIDADE', 'CIDADAO', 'CONTRIBUINTE'] },
  ...
  { id: 'feir', nome: 'FEIRAS', url: '', img: 'tent', acao: 'view', view: 'feiras', perfisPermitidos: ['SERVIDOR', 'CONTRIBUINTE'] },
  ...
  { id: 'setores', nome: 'SETORES', url: '', img: 'https://wcbzmpnvcjamlgljsksk.supabase.co/storage/v1/object/public/public-assets/processos.avif', acao: 'view', view: 'processos', badgetext: 'SHEETS', somenteMaster: true, perfisPermitidos: ['SERVIDOR'] }
];

export default function App() {
  // Navigation & View
  const [currentView, setCurrentView] = useState<'home' | 'demandas' | 'demandas_fiscal' | 'demandas_diretor' | 'feiras' | 'agenda' | 'master' | 'fiscalizacao' | 'processos' | 'processos_lab' | 'laboratorio' | 'cidadao' | 'cnae' | 'telefone' | 'pasta_visa' | 'alvara_visa'>('home');
  ...
  const [loginInitialTab, setLoginInitialTab] = useState<'servidor' | 'contabilidade' | 'contribuinte' | 'cidadao'>('servidor');
  const [loginInitialCidadaoMode, setLoginInitialCidadaoMode] = useState<'login' | 'cadastro'>('login');
  ...
  const [cadastroContribuinteOpen, setCadastroContribuinteOpen] = useState(false);

  const handleOpenLoginModal = (tab: 'servidor' | 'contabilidade' | 'contribuinte' | 'cidadao' = 'servidor', cidadaoMode: 'login' | 'cadastro' = 'login') => {
    setLoginInitialTab(tab);
    setLoginInitialCidadaoMode(cidadaoMode);
    setLoginOpen(true);
  };
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-950 flex flex-col h-screen overflow-hidden text-slate-900 dark:text-slate-100 font-sans text-center">
            ...
            <div className="flex-1 px-1 sm:px-2 md:px-3 lg:px-4 py-2 sm:py-3 pb-28">
              ...
              {currentView === 'cidadao' && (
                <CidadaoView
                  ...
                  onOpenCadastroContribuinte={() => setCadastroContribuinteOpen(true)}
                  onOpenCadastroContabilidade={() => setCadastroContabilidadeOpen(true)}
              ...
              {(currentView === 'demandas_fiscal' || currentView === 'demandas_diretor' || currentView === 'demandas') && (
                <DemandasFiscaisView
                  processos={processos}
                  currentUser={currentUser}
                  users={users}
                  onSaveProcesso={handleSaveProcesso}
                  onNavigate={(v) => setCurrentView(v as any)}
                  initialModo={currentView === 'demandas_diretor' ? 'painel_diretoria' : 'minhas_demandas'}
                />
              )}
      ...
      <LoginModal
        ...
        initialCidadaoMode={loginInitialCidadaoMode}
        onLoginSuccess={(u) => {
          setCurrentUser(u);
          setLoginOpen(false);
        ...
        onOpenCadastroContribuinte={() => {
          setCadastroContribuinteOpen(true);
        }}
      />
