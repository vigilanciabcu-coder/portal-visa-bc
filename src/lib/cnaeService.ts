...
import { CnaeItem, CnaeGrouped, CnaeOption, CnaeRiskDetail } from '../types';
...
/**
 * Consulta um CNAE no banco de dados (Supabase ou Cache) e retorna seus detalhes e risco
 */
export function lookupCnaeInDatabase(rawCnae: string, db?: CnaeItem[]): CnaeRiskDetail {
  ...
  // Fonte de consulta
  const activeDb = (db && db.length > 0) ? db : (getCachedCnaes().length > 0 ? getCachedCnaes() : INITIAL_CNAE_MOCK);

  // 1. Busca por código de 7 dígitos exato
  let matchingItems = cnaeCode7.length === 7
    ? activeDb.filter((it) => (it.subclasse || '').replace(/\D/g, '') === cnaeCode7)
    : [];
  ...
  if (matchingItems.length === 0 && cleanInput) {
    ...
    matchingItems = activeDb.filter((it) => {
      ...
      return (
        (formattedCode && itSub === formattedCode) ||
        (cnaeCode7.length >= 4 && (it.subclasse || '').replace(/\D/g, '').startsWith(cnaeCode7))
      );
    });
  }

  // Se encontrou no banco do Supabase
  if (matchingItems.length > 0) {
    // Escolhe a denominação oficial (sem ser a pergunta com '?')
    ...
    const obs = matchingItems
      .map(it => it.observacao_variavel)
      .filter(Boolean)
      .join(' | ');

    return {
      codigo: formatCnaeCode(itemOficial.subclasse) || itemOficial.subclasse,
      denominacao: itemOficial.denominacao,
      risco: highestRisk,
  ...
  return {
    codigo: cnaeCode7.length === 7 ? formatCnaeCode(cnaeCode7) : codePart,
    denominacao: descPart || codePart,
    risco: fallbackRisco,
    isPrincipal: false,
    origemSupabase: false,
    raw: cleanInput
  };
}

/**
 * Calcula a classificação de risco sanitário oficial do estabelecimento com base nos CNAEs
 * utilizando os dados da tabela Supabase CNAE
 */
export function calculateSanitaryRiskFromCnaes(
  cnaeList: string[],
  db?: CnaeItem[]
): {
  grauRisco: 'ALTO RISCO' | 'MÉDIO RISCO' | 'BAIXO RISCO';
  detalhes: CnaeRiskDetail[];
} {
  ...
  const detalhes: CnaeRiskDetail[] = cnaeList.map((raw, idx) => {
    const detail = lookupCnaeInDatabase(raw, db);
    return {
      ...detail,
      isPrincipal: idx === 0
    };
  ...
  // Regra sanitária do Decreto/Vigilância Sanitária:
  // Se houver qualquer CNAE de ALTO RISCO -> ALTO RISCO
  // Senão, se houver qualquer CNAE de MÉDIO RISCO ou A DEFINIR -> MÉDIO RISCO
  // Senão -> BAIXO RISCO
  const hasAlto = detalhes.some(d => d.risco === 'ALTO RISCO');
  const hasMedio = detalhes.some(d => d.risco === 'MÉDIO RISCO' || d.risco === 'A DEFINIR');

  let grauRisco: 'ALTO RISCO' | 'MÉDIO RISCO' | 'BAIXO RISCO' = 'BAIXO RISCO';
  if (hasAlto) {
    grauRisco = 'ALTO RISCO';
  } else if (hasMedio) {
    grauRisco = 'MÉDIO RISCO';
  } else {
    grauRisco = 'BAIXO RISCO';
  }
