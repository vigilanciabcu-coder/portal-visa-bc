...
// ==========================================
// 5.1. HABITE-SE SANITÁRIO (LC Nº 40/2019)
// ==========================================

export async function saveHabiteSeToSupabase(habiteSeData: {
  ...
  [key: string]: any;
}): Promise<{ success: boolean; error?: string; targetTable?: string }> {
  ...
  try {
    ...
    const obsTexto = `[REQUERIMENTO DE HABITE-SE SANITÁRIO - LC 40/2019]
Protocolo Oficial: ${habiteSeData.numeroDocumento} (${habiteSeData.tipoPeticao || 'TOTAL'})
Edificação: ${habiteSeData.nomeEdificacao || ''}
DIC: ${habiteSeData.numeroDic || 'N/I'}
Endereço: ${habiteSeData.rua || ''}, nº ${habiteSeData.numero || ''} ${habiteSeData.complemento || ''}, ${habiteSeData.bairro || 'Centro'} - Balneário Camboriú/SC
Titular: ${habiteSeData.titularNome || ''} (Doc: ${habiteSeData.titularDoc || ''})
Responsável Técnico: ${habiteSeData.rtNome || ''} (${habiteSeData.rtDoc || ''} - ${habiteSeData.rtRegistroProfissional || ''})
Áreas: ${[
  habiteSeData.usoResidencial && `Residencial: ${habiteSeData.areaResidencial || '—'}m²`,
  habiteSeData.usoComercial && `Comercial: ${habiteSeData.areaComercial || '—'}m²`
].filter(Boolean).join(' • ') || 'Não especificado'}
Alvará de Construção: ${habiteSeData.alvaraConstrucaoNome || 'Em anexo'}
Prioridade: ${habiteSeData.atendimentoPrioritario || 'Não possui'}
Destino: Setor de Habite-se Sanitário (Vistoria Técnica de Engenharia)`;

    // 1. Salva na tabela dedicada 'habite_se_sanitario' (tabela especializada de obras/habite-se)
    let savedInDedicated = false;
    try {
      const enderecoCompleto = `${habiteSeData.rua || ''}, nº ${habiteSeData.numero || ''} ${habiteSeData.complemento || ''}`.trim();
      const baseDedicatedPayload = {
        numero_documento: habiteSeData.numeroDocumento,
        tipo_peticao: habiteSeData.tipoPeticao || 'TOTAL',
        titular_doc: habiteSeData.titularDoc || '',
        titular_nome: habiteSeData.titularNome || '',
        nome_edificacao: habiteSeData.nomeEdificacao || '',
        endereco: enderecoCompleto || null,
        bairro: habiteSeData.bairro || 'Centro',
        numero_dic: habiteSeData.numeroDic || null,
        rt_nome: habiteSeData.rtNome || null,
        rt_doc: habiteSeData.rtDoc || null,
        rt_registro: habiteSeData.rtRegistroProfissional || null,
        alvara_construcao: habiteSeData.alvaraConstrucaoNome || null,
        area_residencial: habiteSeData.areaResidencial || null,
        area_comercial: habiteSeData.areaComercial || null,
        status: 'EM ANÁLISE',
        setor_destino: 'Setor de Habite-se Sanitário'
      };

      const { error: dedErr } = await supabase
        .from('habite_se_sanitario')
        .upsert(baseDedicatedPayload, { onConflict: 'numero_documento' });

      if (!dedErr) {
        savedInDedicated = true;
      } else {
        console.warn('Tentativa de salvar em habite_se_sanitario:', dedErr.message);
      }
    } catch (err: any) {
      console.warn('Aviso tabela dedicada habite_se_sanitario:', err?.message);
    }

    // 2. Salva com garantia total na tabela central 'processos'
    const processoPayload = {
      num_processo: habiteSeData.numeroDocumento,
      data_protocolo: safeDateForPg(habiteSeData.dataHoraEnvio) || dataAtual,
      cnpj_cpf: habiteSeData.titularDoc || '',
      razao_social: habiteSeData.titularNome || '',
      nome_fantasia: habiteSeData.nomeEdificacao || habiteSeData.titularNome || '',
      assunto: `Habite-se Sanitário (${habiteSeData.tipoPeticao || 'TOTAL'})`,
      setor: 'HABITE-SE SANITÁRIO',
      bairro: habiteSeData.bairro || 'Centro',
      endereco: `${habiteSeData.rua || ''}, nº ${habiteSeData.numero || ''} ${habiteSeData.complemento || ''}`.trim(),
    ...
    const { error: procError } = await supabase
      .from('processos')
      .upsert(processoPayload, { onConflict: 'num_processo' });

    if (procError && !savedInDedicated) {
      console.warn('Erro ao salvar Habite-se na tabela processos do Supabase:', procError.message);
      return { success: false, error: procError.message };
    }

    return { success: true, targetTable: savedInDedicated ? 'habite_se_sanitario' : 'processos' };
  } catch (err: any) {
    console.error('Exceção ao salvar Habite-se no Supabase:', err);
    return { success: false, error: err.message || 'Erro inesperado' };
  }
}

export async function fetchHabiteSeFromSupabase(titularDoc?: string) {
  if (!isSupabaseConfigured || !supabase) return [];
  try {
    // 1. Tenta buscar da tabela dedicada 'habite_se_sanitario'
    try {
      let queryDed = supabase.from('habite_se_sanitario').select('*');
      if (titularDoc) {
        queryDed = queryDed.eq('titular_doc', titularDoc);
      }
      const { data: dedData, error: dedErr } = await queryDed.order('created_at', { ascending: false });
      if (!dedErr && dedData && dedData.length > 0) {
        return dedData;
      }
    } catch {
      // Ignora se tabela dedicada ainda não foi criada
    }

    // 2. Fallback para a tabela central 'processos'
    let query = supabase.from('processos').select('*').eq('setor', 'HABITE-SE SANITÁRIO');
    if (titularDoc) {
      query = query.eq('cnpj_cpf', titularDoc);
    }
    const { data, error } = await query.order('created_at', { ascending: false });
    if (error) {
      console.warn('Erro ao buscar Habite-se do Supabase:', error.message);
      return [];
    }
    return data || [];
  } catch {
