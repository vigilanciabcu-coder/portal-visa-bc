...
// Proxy para obter o HTML do Modelo do Google Docs sem bloqueio de CORS
app.get('/api/alvara/doc-template', async (req, res) => {
  try {
    const docUrl = (req.query.url as string) || '';
    if (!docUrl) {
      res.status(400).json({ error: 'URL do documento não informada' });
      return;
    }

    const docMatch = docUrl.match(/\/document\/(?:u\/\d+\/)?d\/([a-zA-Z0-9_-]+)/);
    if (!docMatch || !docMatch[1]) {
      res.status(400).json({ error: 'ID do Google Docs não encontrado na URL' });
      return;
    }
    ...
    if (!response.ok) {
      res.status(response.status).json({ error: `Falha ao exportar modelo do Google Docs (status ${response.status})` });
      return;
    }
    ...
    res.status(500).json({ error: err.message || 'Erro ao obter modelo do Google Docs' });
  }
});

// 7. Controle do Número de Documento Oficial do Habite-se Sanitário (Zero Duplicidade e Reinício Anual)
const HABITE_SE_DATA_FILE = path.join(process.cwd(), 'data', 'habite_se_sequence.json');
const INITIAL_HABITE_SE_SEQ_2026 = 49906;

interface HabiteSeRecord {
  ...
  tramitacoes?: Array<{ dataHora: string; status: string; despacho: string; responsavel: string }>;
}

interface HabiteSeSequenceState {
  ultimoSequencial: number;
  ano: number;
  historico: HabiteSeRecord[];
}

function loadHabiteSeSequence(): HabiteSeSequenceState {
  const anoAtual = new Date().getFullYear();
  let state: HabiteSeSequenceState | null = null;

  try {
    if (fs.existsSync(HABITE_SE_DATA_FILE)) {
      const raw = fs.readFileSync(HABITE_SE_DATA_FILE, 'utf-8');
      const data = JSON.parse(raw);
      if (typeof data.ultimoSequencial === 'number' && typeof data.ano === 'number' && data.ultimoSequencial >= 1) {
        state = {
          ultimoSequencial: data.ultimoSequencial,
          ano: data.ano,
    ...
    console.error('Erro ao ler habite_se_sequence.json:', err);
  }

  if (!state) {
    state = {
      ultimoSequencial: anoAtual === 2026 ? INITIAL_HABITE_SE_SEQ_2026 : 1,
      ano: anoAtual,
      historico: []
    };
    saveHabiteSeSequence(state);
    return state;
  }

  // VIRADA DO ANO: Se o ano atual for maior que o ano registrado no arquivo,
  // o ano é atualizado para o ano atual e o contador reinicia no primeiro número 1
  // para os novos documentos emitidos a partir deste momento.
  if (anoAtual > state.ano) {
    console.log(`[Habite-se Sanitário] Virada de ano detectada (${state.ano} -> ${anoAtual}). Reiniciando sequencial em 1 para novos documentos.`);
    state.ano = anoAtual;
    state.ultimoSequencial = 1;
    saveHabiteSeSequence(state);
  }
...
function saveHabiteSeSequence(state: HabiteSeSequenceState) {
  try {
    const dir = path.dirname(HABITE_SE_DATA_FILE);
    if (!fs.existsSync(dir)) {
      fs.mkdirSync(dir, { recursive: true });
    }
    fs.writeFileSync(HABITE_SE_DATA_FILE, JSON.stringify(state, null, 2), 'utf-8');
  } catch (err) {
    console.error('Erro ao salvar habite_se_sequence.json:', err);
  }
}

function formatarNumeroHabiteSe(seq: number, ano: number): string {
  return `${seq.toLocaleString('pt-BR')}/${ano}`;
}

// Retorna o próximo número disponível do servidor (para visualização no modal)
app.get('/api/habite-se/proximo-numero', (_req, res) => {
  const state = loadHabiteSeSequence();
  const seq = state.ultimoSequencial;
  res.json({
    sucesso: true,
    numeroDocumento: formatarNumeroHabiteSe(seq, state.ano),
    sequencial: seq,
    ano: state.ano,
    origem: 'servidor'
  });
...
// Aloca e confirma definitivamente um número único para o protocolo (evitando duplicatas)
app.post('/api/habite-se/alocar-numero', (req, res) => {
  const state = loadHabiteSeSequence();
  const seqAtual = state.ultimoSequencial;
  const anoDoc = state.ano;
  const numeroFormatado = formatarNumeroHabiteSe(seqAtual, anoDoc);

  const novoRegistro: HabiteSeRecord = {
    ...
    tramitacoes: [
      {
        dataHora: new Date().toISOString(),
        status: 'ENCAMINHADO_SETOR_HABITE_SE',
        despacho: 'Requerimento protocolado eletronicamente e encaminhado para análise técnica do Setor de Habite-se Sanitário.',
        responsavel: 'Protocolo Geral Eletrônico'
      }
    ],
    ...req.body,
    // Garante que o número oficial formatado e sequencial prevaleçam
    numeroDocumento: numeroFormatado,
    sequencial: seqAtual,
    ano: anoDoc
  };

  state.historico.push(novoRegistro);
  // Avança o sequencial do servidor para o próximo requerimento
  state.ultimoSequencial = seqAtual + 1;

  saveHabiteSeSequence(state);

  console.log(`[Habite-se Sanitário] Requerimento nº ${numeroFormatado} alocado e registrado com sucesso.`);
  ...
  res.json({
    ...
    sequencial: seqAtual,
    ano: anoDoc,
    proximoSequencial: state.ultimoSequencial,
    dataHora: novoRegistro.dataHora,
    registro: novoRegistro
  });
});

// Consulta histórico oficial de Habite-se Sanitário (geral ou filtrado por CPF/CNPJ)
app.get('/api/habite-se/historico', (req, res) => {
  ...
  if (doc) {
    ...
    const filtrado = state.historico.filter(h => {
...
// Despacha ou tramita um requerimento para o Setor de Habite-se Sanitário
app.post('/api/habite-se/tramitar', (req, res) => {
  const { numeroDocumento, novoStatus, despacho, responsavel } = req.body;
  const state = loadHabiteSeSequence();
  const registro = state.historico.find(h => h.numeroDocumento === numeroDocumento);

  if (!registro) {
    return res.status(404).json({ sucesso: false, erro: 'Requerimento de Habite-se não encontrado.' });
  }

  registro.status = novoStatus || registro.status || 'EM_ANALISE_HABITE_SE';
  if (!Array.isArray(registro.tramitacoes)) {
    registro.tramitacoes = [];
  }

  registro.tramitacoes.push({
    dataHora: new Date().toISOString(),
    status: novoStatus || 'TRAMITADO',
    despacho: despacho || 'Despachado ao Setor de Habite-se Sanitário',
